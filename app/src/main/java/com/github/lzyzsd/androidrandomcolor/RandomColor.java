...
public class RandomColor {
    ...
    public int randomColor(Color color) {
        ...
        System.out.println("---------------------#" + Integer.toHexString(colorValue) + "hue: " + hue + "  saturation: " + saturation + "  brightness: " + brightness);
        return colorValue;
    }
    ...
    private int pickSaturation(int hue, SaturationType saturationType, Luminosity luminosity) {
        ...
        Range saturationRange = getSaturationRange(hue);
    ...
    private int pickSaturation(Color color, SaturationType saturationType, Luminosity luminosity) {
        ...
        Range saturationRange = getSaturationRange(color);
        if (saturationRange == null) {
            return 0;
        }
    ...
    @DebugLog
    private Range getSaturationRange(int value) {
        ColorInfo colorInfo = getColorInfo(value);
        if (colorInfo != null) {
    ...
    @DebugLog
    private Range getSaturationRange(Color color) {
    ...
    private int pickBrightness(int hue, int saturation, Luminosity luminosity) {
        int min = getMinimumBrightness(hue, saturation),
            max = 100;
    ...
    private int pickBrightness(Color color, int saturation, Luminosity luminosity) {
        int min = getMinimumBrightness(color, saturation),
            max = 100;
    ...
    private int getMinimumBrightness(int hue, int saturation) {
        List<Range> lowerBounds = getColorInfo(hue).getLowerBounds();
    ...
    private int getMinimumBrightness(Color color, int saturation) {
        ColorInfo colorInfo = colors.get(color.name());
        if (colorInfo == null) {
            return 0;
        }
